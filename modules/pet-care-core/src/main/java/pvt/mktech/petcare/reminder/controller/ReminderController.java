package pvt.mktech.petcare.reminder.controller;
...
public class ReminderController {
    ...
    public SseEmitter sseConnect() {
        ...
        try {
            ...
            throw new RuntimeException("SSE 连接建立失败", e);
        }
        ...
        heartbeatTaskRef[0] = sseHeartbeatThreadPoolExecutor.scheduleAtFixedRate(() -> {
            try {
                ...
                if (msg != null && (msg.contains("Broken pipe") || msg.contains("Connection reset"))) {
                    log.debug("SSE 客户端已断开: {}", msg);
                } else {
                    log.debug("SSE 心跳发送失败: {}", msg);
                }
                // 取消心跳任务
